...
class BasePlot:
    ...
    def set_range_self(self):
        ...
        if UiGlobalVariable.GraphScreen == 0:
            ...
            if isinstance(row["HI_LIMIT_TYPE"], float):
                h_limit = row["MAX"]
            step = row["STD"] * 2 if row["STD"] > 0 and not np.isnan(row["STD"]) else 1
            # 处理limit相同的情况
            if l_limit == h_limit:
                offset = abs(l_limit) * 0.1 if abs(l_limit) > 0 else 1
                self.set_p_range(l_limit - offset, h_limit + offset)
            else:
        ...
        if UiGlobalVariable.GraphScreen == 1:
            ...
            # 处理min和max相同的情况
            if p_min == p_max:
                offset = abs(p_min) * 0.1 if abs(p_min) > 0 else 1
                p_min, p_max = p_min - offset, p_max + offset
            self.set_p_range(p_min, p_max)
            return
        if UiGlobalVariable.GraphScreen == 2:
            ...
            # 处理min和max相同的情况
            if low == high:
                offset = abs(low) * 0.1 if abs(low) > 0 else 1
        ...
        if UiGlobalVariable.GraphScreen == 3:
            if row["STD"] == 0 or np.isnan(row["STD"]):
                ...
                step = row["STD"] * UiGlobalVariable.GraphMeanAddSubSigma
    ...
    def set_p_range(self, p_min, p_max):
        ...
        if p_max <= p_min:
            if p_min == 0:
                ...
                offset = abs(p_min) * 0.1 if abs(p_min) > 0 else 0.1
                p_min, p_max = p_min - offset, p_min + offset
