...
class Main_Ui(QMainWindow, Ui_MainWindow):
    ...
    @Slot()
    def on_action_open_temp_folder_triggered(self):
        """
        打开TEMP文件夹 C:\1_STDF
        """
        temp_folder = r"C:\1_STDF"
        if os.path.exists(temp_folder):
            subprocess.Popen(f'explorer "{temp_folder}"')
            self.mdi_space_message_emit("已打开TEMP文件夹: C:\\1_STDF")
        else:
            self.mdi_space_message_emit("TEMP文件夹不存在: C:\\1_STDF")
    ...
    def on_action_email_triggered(self):
        if self.message_show(
                'mail.To = "1223858490@qq.com"\r\nmail.Cc = "1223858490@qq.com"\r\n点击确认使用outlook反馈问题'
        ):
    ...
    def on_action_linear_triggered(self, script_name='jmp_point_data'):
        ...
        # 显示可用的TEST_ID列表供用户参考
        available_test_ids = list(mdi.li.capability_key_dict.keys())
        available_test_ids_str = ", ".join([str(tid) for tid in sorted(available_test_ids)[:10]])  # 显示前10个
        if len(available_test_ids) > 10:
            available_test_ids_str += "..."

        # 获取用户输入的TEST_ID
        dialog_text = f"请填写与所选数据做相关对比的数据首列 TEST_ID\n\n" \
                     f"当前已选择的测试项目: {test_id_list}\n" \
                     f"可用的TEST_ID示例: {available_test_ids_str}\n\n" \
                     f"请输入TEST_ID (纯数字，如: 10001):"

        remark, ok = QInputDialog.getText(self, "填写对比列", dialog_text)

        if not ok or not remark.strip():
            return

        # 验证输入格式
        try:
            remark = int(remark.strip())
        except ValueError:
            return self.message_show(f"输入格式错误！\n\n"
                                   f"请输入纯数字格式的TEST_ID\n"
                                   f"示例: 10001, 130, 1001\n"
                                   f"您输入的是: '{remark}'")
        ...
        if remark not in mdi.li.capability_key_dict.keys():
            return self.message_show(f"输入的TEST_ID不存在！\n\n"
                                   f"您输入的TEST_ID: {remark}\n"
                                   f"可用的TEST_ID数量: {len(available_test_ids)}\n"
                                   f"请从数据表格中选择一个有效的TEST_ID")

        # 如果输入的TEST_ID不在已选择的列表中，则添加它
        if remark not in test_id_list:
            test_id_list.append(remark)
            self.mdi_space_message_emit(f"已将TEST_ID {remark} 添加到分析列表中")

        # 获取数据进行分析
        try:
            jmp_df, temp_calculation = mdi.li.get_unstack_data_to_csv_or_jmp_or_altair(test_id_list)
        except Exception as e:
            return self.message_show(f"数据获取失败: {str(e)}")
        ...
        # 生成JMP脚本
        try:
            # 修复：使用mdi.li而不是self.li
            reference_text = mdi.li.get_text_by_test_id(remark)
            jmp_script = JmpScript.factory(
                JmpFile.load_csv_file(fit_csv_path),
            ...
            self.mdi_space_message_emit(f"线性回归分析完成！参考列: {reference_text} (TEST_ID: {remark})")
        except Exception as e:
            return self.message_show(f"JMP脚本生成失败: {str(e)}")
